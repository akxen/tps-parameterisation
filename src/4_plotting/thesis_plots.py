...
class PlotHandler:
    ...
    def plot_weighted_rnn_average_price(self, angle_limit):
        ...
        fig.savefig(os.path.join(self.output_dir, 'figures', 'thesis', f'{filename}.pdf'))
        plt.show()
    ...
    def plot_price_target_results(self, angle_limit):
        ...
        fig.savefig(os.path.join(self.output_dir, 'figures', 'thesis', f'{filename}.pdf'))
        fig.savefig(os.path.join(self.output_dir, 'figures', 'thesis', f'{filename}.png'), dpi=300)
    ...
    def plot_average_price_target(self, angle_limit):
        """Show price targets"""
        ...
        # Average electricity prices for different emissions intensity baselines
        file_path = os.path.join(self.output_dir, 'figures', 'thesis', 'tmp', 'df_average_prices.pickle')
        try:
            df_average_prices = pd.read_pickle(file_path)
        except:
            df_average_prices = self.get_average_prices(df_fixed_baseline, category='FIXED_BASELINE')
            df_average_prices.to_pickle(file_path)
        ...
        # Average price for each fixed baseline scenario
        ax.plot(df_average_prices.loc['NATIONAL'].div(bau_price).index,
                df_average_prices.loc['NATIONAL'].div(bau_price).values,
                linestyle='-',
                color='#1227e3',
                linewidth=0.75,
        ...
        ax.scatter(df_average_prices.loc['NATIONAL'].div(bau_price).index,
                   df_average_prices.loc['NATIONAL'].div(bau_price).values,
                   marker='o',
                   facecolors='none',
                   edgecolors='#1227e3',
        ...
        # Baselines for given weighted RRN price target
        ax.scatter(df_1['PHI_DISCRETE'], df_1['AVERAGE_PRICE_BAU_MULTIPLE'],
                   marker='o',
                   facecolors='#ed0505',
                   edgecolors='#ed0505',
                   linewidths=0.5,
    ...
    def plot_average_price_target_error(self, angle_limit):
        """Show price targets"""
        ...
        # Weighted RRN prices and their corresponding price targets
        ax.scatter(df_1['WEIGHTED_RRN_PRICE_TARGET_BAU_MULTIPLE'], df_1['AVERAGE_PRICE_BAU_MULTIPLE'],
                   marker='o',
                   facecolors='#ed0505',
                   edgecolors='#ed0505',
                   linewidths=0.5,
        ...
        # Straight line with slope = 1
        ax.plot([0.8, 1.45], [0.8, 1.45], linestyle=':', linewidth=0.8, color='k', alpha=0.8)
        ...
        fig.set_size_inches(self.cm_to_in(14), self.cm_to_in(14) / 1.8)
        fig.subplots_adjust(left=0.1, bottom=0.15, right=0.98, top=0.98)
        filename = f'average_price_target_error_{int(float(angle_limit) * 1000)}'
    ...
    def plot_permit_price_target(self, angle_limit):
        """Show price targets"""

        # Load fixed baseline results
        fixed_baseline = self.load_results(f'df_fixed_baseline_angle_limit_{angle_limit}.pickle')
        permit_price_target = self.load_results(f'df_permit_price_target_angle_limit_{angle_limit}.pickle')

        # Permit price as a function of emissions intensity baseline
        tau = self.permit_price_vs_baseline(fixed_baseline, category='FIXED_BASELINE')

        # Permit price targeting scenarios
        df = permit_price_target.groupby('PERMIT_PRICE_TARGET')[['Variable']].apply(
            lambda x: x.loc[['tau', 'PHI_DISCRETE']]).unstack().applymap(lambda x: x['Value'])
        df.columns = df.columns.droplevel(0)

        # Multiply by 100 to account for prior normalisation
        df['tau'] = df['tau'].mul(100)

        # Initialise figure
        fig, ax = plt.subplots()

        for index, row in df.iterrows():
            ax.plot([0.9, 1.1], [index * 100, index * 100], linestyle=':', linewidth=0.5, color='k')

        # Permit prices
        tau = tau.sort_index()
        # ax.plot(tau.index, tau.values, color='#2609e0')

        ax.plot(tau.index, tau.values,
                linestyle='-',
                color='#d60909',
                linewidth=0.75,
                alpha=0.8,
        ...
        ax.scatter(tau.index, tau.values,
                   marker='o',
                   facecolors='none',
                   edgecolors='#d60909',
                   linewidths=0.5,
        ...
        # Permit prices for given permit price target
        ax.scatter(df['PHI_DISCRETE'], df['tau'],
                   marker='o',
                   facecolors='#2609e0',
                   edgecolors='#2609e0',
                   linewidths=0.5,
    ...
    def plot_permit_price_target_error(self, angle_limit):
        """Show price targets"""
        ...
        # Permit price targeting scenarios
        permit_prices = permit_price_target.groupby('PERMIT_PRICE_TARGET')[['Variable']].apply(
            lambda x: x.loc[['tau', 'PHI_DISCRETE']]).unstack().applymap(lambda x: x['Value'])
        permit_prices.columns = permit_prices.columns.droplevel(0)

        # Multiply by 100 to account for prior normalisation
        permit_prices['tau'] = permit_prices['tau'].mul(100)

        # Initialise figure
        fig, ax = plt.subplots()

        # Difference between permit price and target
        ax.scatter(permit_prices.reset_index()['PERMIT_PRICE_TARGET'].mul(100), permit_prices['tau'],
                   marker='o',
                   facecolors='#2609e0',
                   edgecolors='#2609e0',
                   linewidths=0.5,
        ...
        # Line with slope = 1
        ax.plot([0, 110], [0, 110], linestyle=':', linewidth=0.8, color='k', alpha=0.8)
        ...
        # Set figure size
        fig.set_size_inches(self.cm_to_in(14), self.cm_to_in(14) / 1.8)
        fig.subplots_adjust(left=0.12, bottom=0.15, right=0.97, top=0.97)

        filename = f'permit_price_target_error_{int(float(angle_limit) * 1000)}'
        fig.savefig(os.path.join(self.output_dir, 'figures', 'thesis', f'{filename}.pdf'))
        fig.savefig(os.path.join(self.output_dir, 'figures', 'thesis', f'{filename}.png'), dpi=300)

        plt.show()

    def plot_weighted_rnn_average_price_error(self, angle_limit):
        """Comparison between weighted RNN price and average price"""
        ...
        file_path = os.path.join(self.output_dir, 'figures', 'thesis', 'tmp', 'df_average_prices.pickle')
        try:
            average_prices = pd.read_pickle(file_path)
        except:
            average_prices = self.get_average_prices(fixed_baseline, category='FIXED_BASELINE')
            average_prices.to_pickle(file_path)

        # Weighted RRN prices for different fixed baseline scenarios
        weighted_rrn_prices = self.get_weighted_rrn_prices(fixed_baseline, category='FIXED_BASELINE')

        # Average BAU price
        bau_price = average_prices.loc['NATIONAL'].iloc[-1]

        # Initialise figure
        fig, ax = plt.subplots()

        # Average price (x-axis) vs weighted RRN price (y-axis)
        ax.scatter(average_prices.loc['NATIONAL'].div(bau_price), weighted_rrn_prices.div(bau_price),
                   marker='o',
                   facecolors='#ed0505',
                   edgecolors='#ed0505',
                   linewidths=0.5,
        ...
        # Line with slope = 1
        ax.plot([0.92, 1.57], [0.92, 1.57], linestyle=':', color='k', alpha=0.8, linewidth=0.8)

        # Axes labels
        ax.set_xlabel('Average price relative to BAU')
        ...
        # Set figure size
        fig.set_size_inches(self.cm_to_in(14), self.cm_to_in(14) / 1.8)
        fig.subplots_adjust(left=0.13, bottom=0.14, right=0.97, top=0.97)

        # Save figure
        filename = f'weighted_rrn_prices_error_{int(float(angle_limit) * 1000)}'
        fig.savefig(os.path.join(self.output_dir, 'figures', 'thesis', f'{filename}.pdf'))
        fig.savefig(os.path.join(self.output_dir, 'figures', 'thesis', f'{filename}.png'), dpi=300)
        plt.show()

    def plot_srmc_impact(self, angle_limit):
        """Plot SRMC bands for each generator type as a function of baseline"""
        ...
        fuel_types = ['Black coal', 'Brown coal', 'Natural Gas (Pipeline)']
        means, weighted_means, max_srmcs, min_srmcs, std_srmcs = self.get_srmc_comparison(tau, self.df_g, fuel_types)

        # SRMC ranges for different generating technologies
        fig, ax = plt.subplots()

        # Colours associated with each generating technology
        s = pd.Series(['#112fd9', '#ce0037', '#039642'], index=['Black coal', 'Brown coal', 'Natural Gas (Pipeline)'])

        # Mean, max, and min SRMCs for different generating technologies
        means.plot(ax=ax, color=s, alpha=1, style='--', legend=False, linewidth=0.5)
        means.add(std_srmcs).plot(ax=ax, style='-', color=s, legend=False, alpha=1, linewidth=0.5)
        means.subtract(std_srmcs).plot(ax=ax, style='-', color=s, legend=False, alpha=1, linewidth=0.5)

        # Fill line between upper and lower standard deviation and mean SRMC
        ax.fill_between(means.index, means.subtract(std_srmcs)['Natural Gas (Pipeline)'],
                        means.add(std_srmcs)['Natural Gas (Pipeline)'], facecolor='#039642', alpha=0.5)
        ax.fill_between(means.index, means.subtract(std_srmcs)['Black coal'], means.add(std_srmcs)['Black coal'],
                        facecolor='#0071b2', alpha=0.5)
        ax.fill_between(means.index, means.subtract(std_srmcs)['Brown coal'], means.add(std_srmcs)['Brown coal'],
                        facecolor='#ce0037', alpha=0.5)
    ...
    def plot_generator_output_impact(self, angle_limit):
        ...
        # Generator output
        generator_output = fixed_baseline.loc[fixed_baseline['variable_name'] == 'p'].apply(
            self._get_generator_output, axis=1)
        ...
        # Colours associated with each generating technology
        gen_colours = zip(['Black coal', 'Brown coal', 'Natural Gas (Pipeline)'], ['#112fd9', '#ce0037', '#039642'])

        # Plot energy output for different technologies
        df = (pd.merge(generator_output, self.df_g[['NEM_REGION', 'FUEL_TYPE', 'FUEL_CAT']],
                       how='left', left_on='DUID', right_index=True)
            .groupby(['FUEL_TYPE', 'FIXED_BASELINE'])['energy_output']
            .sum().reset_index()
            .pivot(index='FIXED_BASELINE',
        ...
        for g, c in gen_colours:
            ax.plot(df[g].index, df[g].values,
                    linestyle='-',
                    color=c,
                    linewidth=0.75,
                    alpha=0.8,
            ...
            ax.scatter(df[g].index, df[g].values,
                       marker='o',
                       facecolors='none',
                       edgecolors=c,
                       linewidths=0.5,
